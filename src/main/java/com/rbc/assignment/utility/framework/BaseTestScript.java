...
/***
 * This is BaseTestScript class which the core of this framework design.
 * All the reusable methods are defined in this class. 
 * @author ashimnanda19
 *
 */
...
/***
 * The setDriver method creates instance of driver based on the input parameter which is passed 
 * on from calling method.
 * @param browser
 */
	protected void setDriver(String browser) {
		...
		try {
			...
			Log.error("Exception Occured while initiating the instance of Webdriver" + e.getMessage());
		}
	...
	protected void seHoverOnElement(WebElement testElement, String elementName) {
		try {
			...
			if (testElement.isDisplayed()) {
				...
				Log.info("Successfully hovered on element");
			} else {
				Log.info("Unable to hover on Element");
			}
		} catch (Exception e) {
	...
	/***
	 * This method help  to select a dropdown Option based on the value passed a parameter.
	 * This method supports only selecting only single value from dropdown.
	 * @param testElement
	 * @param elementName
	 * @param desiredValue
	 */
	...
	/***
	 * This method waits for a clickable Webelement passed on as parameter for a given time. 
	 * @param driver
	 * @param testElement
	...
	/***
	 * This method Explicitly waits till visibility of  a given WebElement for a given time.
	 * @param driver
	 * @param testElement
	 * @param maxTimeOutInSeconds
	 */
	...
	protected boolean seCompareText(WebElement testElement,String elementName ,String expectedValue) {
		...
		try {
			...
			if(actualValue.equalsIgnoreCase(expectedValue)) {
				isMatch=true;
				Log.info("Compare Text: Expected Value: "+expectedValue+" is matching with Actual Value: "+actualValue+" For WebElement: "+elementName);
			}else {
				Log.info("Compare Text: Expected Value: "+expectedValue+" does not match with Actual Value: "+actualValue+" For WebElement: "+elementName);
			}
		} catch (Exception e) {
